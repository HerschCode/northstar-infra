...
| **Artifact Registry** | 0.5 GiB storage, then about $0.10 per GiB-month | 3 images. Python images with numpy / onnx / mlflow can be ~1 GB each, and operations-assistant's requirements pull in torch (roughly 2 GB of torch and CUDA libraries on Linux, see [app-integration.md](app-integration.md#not-done-and-known-gaps)); the cleanup policy keeps the newest 3 versions of each, and versions share unchanged layers, so three versions cost far less than three times one | **the one line that can cost money**: roughly $0.25–0.90 a month if images are large, more if operations-assistant's is not slimmed. Unknown until real images exist |
